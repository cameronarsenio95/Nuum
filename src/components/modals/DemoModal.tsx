...
export function DemoModal({ isOpen, onClose }: DemoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="border rounded-linear-lg max-w-5xl w-full relative dark:bg-linear-bg-secondary dark:border-linear-border light:bg-white light:border-gray-300 light:shadow-2xl">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full flex items-center justify-center linear-transition dark:bg-linear-bg-hover dark:hover:bg-linear-bg-active dark:text-text-secondary dark:hover:text-text-primary light:bg-gray-100 light:hover:bg-gray-200 light:text-text-light-secondary light:hover:text-text-light-primary"
        >
          <X className="w-5 h-5" />
        </button>
