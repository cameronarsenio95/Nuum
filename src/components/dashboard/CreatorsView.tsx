...
export function CreatorsView({ workspace }: CreatorsViewProps) {
  ...
  if (loading) {
    return <div className="dark:text-text-secondary light:text-text-light-secondary">Loading creators...</div>;
  }
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
        <div className="flex-1">
          ...
          <p className="dark:text-text-secondary light:text-text-light-secondary">Manage your influencer database</p>
              ...
              <div className="flex flex-wrap gap-2">
                {creator.instagram_handle && (
                  <a
                    ...
                    className="flex items-center gap-1 px-2 py-1 dark:bg-linear-bg-subtle light:bg-linear-light-bg-subtle rounded-linear text-xs dark:text-text-secondary light:text-text-light-secondary hover:dark:text-text-primary hover:light:text-text-light-primary linear-transition"
                  >
              ...
              {creator.tags && creator.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-3">
                  {creator.tags.slice(0, 3).map((tag, i) => (
                    <span key={i} className="px-2 py-0.5 bg-linear-info/10 text-linear-info text-xs rounded-full">
                      {tag}
                    </span>
                  ))}
                  {creator.tags.length > 3 && (
                    <span className="px-2 py-0.5 dark:text-text-tertiary light:text-text-light-tertiary text-xs">
                      +{creator.tags.length - 3}
                    </span>
                  )}
                </div>
