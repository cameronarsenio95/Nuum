...
export function TrialBanner({ daysRemaining, onUpgradeClick }: TrialBannerProps) {
  const { freeAccountInfo, trialInfo } = usePlanLimits();
  const [isDismissed, setIsDismissed] = useState(false);

  if (isDismissed) return null;

  if (freeAccountInfo.isFreePlan && !freeAccountInfo.isFrozen && freeAccountInfo.daysRemaining <= 3) {
    const isUrgent = freeAccountInfo.daysRemaining <= 2;
        ...
        <div className="flex items-start gap-4 pr-8">
          ...
          <div className="flex-1">
            <h3 className="text-base font-medium mb-1">
              {isUrgent ? 'Free Plan Expiring Soon!' : 'Free Plan Expiring in 3 Days'}
            </h3>
            <p className="text-sm dark:text-text-secondary light:text-text-light-secondary mb-3">
              {isUrgent ? (
                <>
                  Only <strong className="text-red-400">{freeAccountInfo.daysRemaining} {freeAccountInfo.daysRemaining === 1 ? 'day' : 'days'}</strong> left before your account is frozen!
                  Upgrade to Standard to keep full access.
                </>
              ) : (
                <>
                  You have <strong>{freeAccountInfo.daysRemaining} days</strong> remaining in your Free plan.
                  Upgrade to Standard plan to avoid account freeze.
                </>
              )}
            </p>
  ...
  if (trialInfo.isActive && daysRemaining !== undefined) {
