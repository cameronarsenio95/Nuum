...
import {
  ...
  ShoppingBag,
  Lock,
  CalendarDays, // 👈 agenda icoon toegevoegd
} from 'lucide-react';
...
export function DashboardLayout({
  ...
  const navigation = [
    ...
    { name: 'Agenda', value: 'agenda' as const, icon: CalendarDays }, // 👈 NIEUW ITEM
  ...
  return (
    ...
    >
      ...
      >
        {isMobileMenuOpen ? (
          <X className="w-6 h-6" style={{ color: NUUM_COLORS.textPrimary }} />
        ) : (
          <Menu className="w-6 h-6" style={{ color: NUUM_COLORS.textPrimary }} />
        )}
      </button>
      ...
      <div className="flex h-screen">
        {/* Sidebar */}
        <aside
          className={`w-64 border-r flex flex-col fixed lg:static inset-y-0 left-0 z-40 transform transition-transform duration-300 ${
            isMobileMenuOpen
              ? 'translate-x-0'
          ...
          >
            <div className="flex items-center gap-3">
              <img
                src={
                  theme === 'dark'
                    ? '/assets/members/nuum - White Mark.png'
                    : '/assets/members/nuum - Black Mark.png'
                }
                alt="NUUM"
              ...
              <div className="min-w-0 flex-1">
                ...
                >
                  {profile?.company || workspace.name}
                </h1>
                <span
                  className="text-xs px-2 py-0.5 rounded-full border capitalize inline-block"
                  style={{
                    color:
                      workspace.plan === 'free'
                        ? '#9CA3AF'
                ...
                return (
                  <li key={item.value} className="relative group">
                    <button
                      ...
                      style={{
                        color: isDisabled
                          ? NUUM_COLORS.textTertiary
                          : isActive
                          ? NUUM_COLORS.textPrimary
                          : NUUM_COLORS.textSecondary,
                        backgroundColor: isActive
                          ? NUUM_COLORS.surfaceHover
                          : 'transparent',
                        borderLeft: isActive
                          ? `2px solid ${NUUM_COLORS.accent}`
                    ...
                    {isDisabled && (
                      <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-red-500 text-white text-xs rounded-linear whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-50 shadow-lg">
            ...
            <div className="flex items-center justify-between mb-2 px-3">
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{ color: NUUM_COLORS.textTertiary }}
                >
                  Theme
                </span>
                <ThemeToggle />
