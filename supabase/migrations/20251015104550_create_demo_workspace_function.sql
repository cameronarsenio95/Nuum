/*
  # Create Demo Workspace Function

  ## Overview
  This migration creates a function that allows workspace owners to create a demo workspace
  populated with sample data based on their own workspace structure.

  ## Functions
  - `create_demo_workspace_from_template` - Creates a demo workspace with sample data
  
  ## Important Notes
  - Demo workspaces are owned by the calling user
  - Sample data is realistic but clearly marked as demo content
  - Function returns the demo workspace ID for link generation
*/
...
BEGIN
  ...
  INSERT INTO campaigns (workspace_id, name, brand, status, start_date, end_date, budget, revenue, description, spark_url)
  VALUES 
    (
      new_workspace_id,
  ...
  INSERT INTO campaigns (workspace_id, name, brand, status, start_date, end_date, budget, revenue, description)
  VALUES 
    (
      new_workspace_id,
  ...
  -- Create sample creators and store their IDs
  WITH inserted_creators AS (
    INSERT INTO creators (workspace_id, name, email, instagram_handle, tiktok_handle, snapchat_handle, follower_count, engagement_rate, niche, status, notes)
    VALUES 
      (new_workspace_id, 'Sarah Martinez', 'sarah@example.com', '@sarahstyle', '@sarahfashion', NULL, 125000, 4.8, 'Fashion & Lifestyle', 'active', 'Specializes in sustainable fashion content'),
      (new_workspace_id, 'Alex Chen', 'alex@example.com', '@alexfitness', '@alexworkouts', 'alexfitpro', 89000, 5.2, 'Fitness & Wellness', 'active', 'Professional fitness trainer'),
      (new_workspace_id, 'Emma Rodriguez', 'emma@example.com', '@emmaskincare', '@glowwithemma', NULL, 156000, 4.5, 'Beauty & Skincare', 'active', 'Skincare specialist'),
      (new_workspace_id, 'Marcus Johnson', 'marcus@example.com', '@marcusstyle', '@mjfashion', 'marcusstyle', 203000, 3.9, 'Menswear & Fashion', 'active', 'Menswear fashion creator'),
      (new_workspace_id, 'Priya Patel', 'priya@example.com', '@priyabeauty', '@priyamakeup', NULL, 178000, 4.7, 'Beauty & Makeup', 'active', 'Makeup artist'),
      (new_workspace_id, 'Jake Williams', 'jake@example.com', '@jakeoutdoors', '@jakewild', 'jakewilliams', 95000, 5.5, 'Outdoor & Adventure', 'pending', 'Adventure creator'),
      (new_workspace_id, 'Nina Foster', 'nina@example.com', '@ninawellness', '@mindfulnina', NULL, 67000, 6.1, 'Wellness & Mindfulness', 'active', 'Wellness coach'),
      (new_workspace_id, 'Tyler Brooks', 'tyler@example.com', '@tylertech', '@techtyler', 'tylertech', 142000, 4.2, 'Tech & Gadgets', 'active', 'Tech reviewer')
    RETURNING id
  )
  ...
  INSERT INTO ad_sets (campaign_id, name, platform, status, budget, spend, impressions, clicks, conversions, revenue, spark_code)
  VALUES 
