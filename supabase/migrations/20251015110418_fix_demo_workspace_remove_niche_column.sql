/*
  # Fix Demo Workspace - Remove Niche Column

  ## Overview
  Updates the create_demo_workspace_from_template function to remove niche column
  which doesn't exist in the creators table
  
  ## Changes
  - Remove 'niche' from the INSERT statement for creators
  - Keep all other creator information intact

  ## Important Notes
  - This is a bug fix for column doesn't exist error
*/
...
BEGIN
  ...
  WITH inserted_creators AS (
    ...
    VALUES 
      (new_workspace_id, 'Sarah Martinez', 'sarah@example.com', '@sarahstyle', '@sarahfashion', NULL, 125000, 4.8, 'active', 'Fashion & Lifestyle - Specializes in sustainable fashion content'),
      (new_workspace_id, 'Alex Chen', 'alex@example.com', '@alexfitness', '@alexworkouts', 'alexfitpro', 89000, 5.2, 'active', 'Fitness & Wellness - Professional fitness trainer'),
      (new_workspace_id, 'Emma Rodriguez', 'emma@example.com', '@emmaskincare', '@glowwithemma', NULL, 156000, 4.5, 'active', 'Beauty & Skincare - Skincare specialist'),
      (new_workspace_id, 'Marcus Johnson', 'marcus@example.com', '@marcusstyle', '@mjfashion', 'marcusstyle', 203000, 3.9, 'active', 'Menswear & Fashion - Menswear fashion creator'),
      (new_workspace_id, 'Priya Patel', 'priya@example.com', '@priyabeauty', '@priyamakeup', NULL, 178000, 4.7, 'active', 'Beauty & Makeup - Makeup artist'),
      (new_workspace_id, 'Jake Williams', 'jake@example.com', '@jakeoutdoors', '@jakewild', 'jakewilliams', 95000, 5.5, 'pending', 'Outdoor & Adventure - Adventure creator'),
      (new_workspace_id, 'Nina Foster', 'nina@example.com', '@ninawellness', '@mindfulnina', NULL, 67000, 6.1, 'active', 'Wellness & Mindfulness - Wellness coach'),
      (new_workspace_id, 'Tyler Brooks', 'tyler@example.com', '@tylertech', '@techtyler', 'tylertech', 142000, 4.2, 'active', 'Tech & Gadgets - Tech reviewer')
    RETURNING id
  )
